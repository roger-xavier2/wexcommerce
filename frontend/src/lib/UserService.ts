  ...
  fetchInstance
    ...
    .then(async (res) => {
      if (res.data.accessToken) {
        (await cookies()).set('wc-fe-user', JSON.stringify(res.data), await getCookieOptions())
      }
...
export const setLanguage = async (lang: string) => {
  (await cookies()).set('wc-fe-language', JSON.stringify(lang), await getCookieOptions())
}
...
export const setStayConnected = async (value: boolean) => {
  (await cookies()).set('wc-stay-connected', JSON.stringify(value), await getCookieOptions())
}
