...
const getClientApiHost = (): string => {
  const configured = env.CLIENT_API_HOST!

  // If you access the app via IP from another device, `localhost` would point to
  // that device. In such cases, reuse the current hostname for the backend.
...
const getURL = (url: string) =>
  `${wexcommerceHelper.trimEnd(env.isServer() ? env.SERVER_API_HOST! : getClientApiHost(), '/')}/${wexcommerceHelper.trimStart(url, '/')}`
