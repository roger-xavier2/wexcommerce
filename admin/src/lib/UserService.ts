  ...
  fetchInstance
    ...
    .then(async (res) => {
      if (res.data.accessToken) {
        (await cookies()).set('wc-be-user', JSON.stringify(res.data), await getCookieOptions())
      }
    ...
    .then(async (res) => {
      if (res.status === 200) {
        ...
        if (user) {
          ...
          cookieStore.set('wc-be-user', JSON.stringify(user), await getCookieOptions())
        }
      }
      return res.status
    })
...
export const setLanguage = async (lang: string) => {
  (await cookies()).set('wc-be-language', JSON.stringify(lang), await getCookieOptions())
}
