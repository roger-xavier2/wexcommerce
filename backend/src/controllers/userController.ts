...
export const deleteUsers = async (req: Request, res: Response) => {
  try {
    ...
    for (const id of ids) {
      ...
      if (user) {
        // Safety: do not allow deleting admin users from this endpoint
        // (use the dedicated reset script/flow for admin accounts).
        if (user.type === wexcommerceTypes.UserType.Admin) {
          logger.warn(`[user.deleteUsers] Skipping admin user deletion: ${user.email ?? id.toString()}`)
          continue
        }

        // Remove auth/verification tokens
        await Token.deleteMany({ user: id })

        // Remove wishlist(s)
        await Wishlist.deleteMany({ user: id })

        // Remove cart(s) and their cart items
        const carts = await Cart.find({ user: id })
        for (const cart of carts) {
          const cartItemIds = (cart.cartItems || []) as mongoose.Types.ObjectId[]
        ...
        // Remove orders and their order items
        const orders = await Order.find({ user: id })
        for (const order of orders) {
          const orderItemIds = (order.orderItems || []) as mongoose.Types.ObjectId[]
...
export const sendEmail = async (req: Request, res: Response) => {
  try {
    ...
    if (!origin || (!authHelper.isAdmin(req) && !authHelper.isFrontend(req))) {
